...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.sin(self.nodes[0, :]) + np.cos(self.nodes[1, :] + self.nodes[2, :])
        print(self.smth)

        # задаем угловую скорость вращения
        omega = 0.3
        ...
        for i in range(len(self.nodes[0])):
            if randint(1, 20) == 1:
                self.velocity[0][i] = uniform(-4, 4)/10
                self.velocity[1][i] = uniform(-4, 4)/10
                self.velocity[2][i] = uniform(-4, 4)/10
    ...
    def move(self, tau, i):
        ...
        self.smth = np.sin(self.nodes[0, :] + 2*pi*tau*i) + np.cos(self.nodes[1, :] + self.nodes[1, :]+ 2*pi*tau*i)
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("my_hammer-step-" + str(snap_number) + ".vtu")
        writer.Write()
